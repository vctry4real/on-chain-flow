import { GetWalletTransfersInput, GetTokenSwapsInput, GetBridgeEventsInput, GetAddressLabelsInput, } from '../schemas/raw-data.js';
...
export function registerRawDataTools(server) {
    ...
    server.tool('get_wallet_transfers', 'Return normalised ERC-20 token transfer history for any wallet across supported chains. Raw building block for custom analysis — used internally by trace_capital_flow.', GetWalletTransfersInput.shape, async (args) => {
        try {
    ...
    // ─── get_token_swaps ─────────────────────────────────────────────────────
    server.tool('get_token_swaps', 'Return normalised DEX swap history for a token across Uniswap V3, Curve, Balancer, and 1inch. Includes per-swap trader labels and price impact — raw feed for custom accumulation analysis.', GetTokenSwapsInput.shape, async (args) => {
        try {
    ...
    // ─── get_bridge_events ───────────────────────────────────────────────────
    server.tool('get_bridge_events', 'Return raw bridge transfer events for a token across Stargate, Across, and Hop Protocol. Provides the underlying transaction-level data consumed by bridge_flow_anomalies.', GetBridgeEventsInput.shape, async (args) => {
        try {
    ...
    // ─── get_address_labels ──────────────────────────────────────────────────
    server.tool('get_address_labels', 'Bulk-resolve up to 50 wallet addresses against the entity label registry. Returns human-readable entity names, categories (CEX / bridge / fund / whale / mixer), and risk scores. Converts anonymous hashes into an intelligence narrative.', GetAddressLabelsInput.shape, async (args) => {
        try {

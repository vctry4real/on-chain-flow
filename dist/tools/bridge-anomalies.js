...
function buildMockBridgeRoutes(token_address, hours) {
...
export function registerBridgeFlowAnomalies(server) {
    server.tool('bridge_flow_anomalies', 'Detect statistically anomalous cross-chain bridge inflows for a token. Flags volume deviations beyond configurable sigma thresholds against a rolling 30-day baseline, identifies responsible wallets, and — uniquely — correlates bridge-receiving wallets against the stealth accumulation watchlist to surface the highest-confidence pre-move signal available anywhere.', BridgeFlowAnomaliesInput.shape, async (args) => {
        try {
            ...
            const accumulationWatchlist = new Set();
            const processedInflows = rawRoutes
                .map((route) => {
                const z = computeZScore(route.volume_usd, route.baseline_mean, route.baseline_std);
                if (z < parsed.sigma_threshold)
                    return null;
            ...
            const correlated_accumulation = processedInflows.some((inf) => inf.receiving_wallets.some((w) => w.also_accumulating));
            ...
            const result = {
                ...
                narrative: generateBridgeNarrative('USDC', verdict, total_anomalous_volume_usd, correlated_accumulation, processedInflows.length),
...
//# sourceMappingURL=bridge-anomalies.js.map
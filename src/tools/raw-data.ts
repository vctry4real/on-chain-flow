...
function mockTransfers(address: string, chain: string, hours: number, limit: number) {
    ...
    async (args) => {
      try {
        ...
        const labels = parsed.addresses.map((addr) => {
          ...
          return known
            ? { address: addr, ...known }
            : { address: addr, label: 'Unknown Wallet', entity_type: 'unknown' as const, tags: [], risk_score: 0.0 };
        });
